...
{
    static $cache_area = 'stock_area';//地区大类缓存标识
    protected $table = 'myhub_stock_area';

    public function _dcache_class(){
        Cache::rm(self::$cache_area);
    }
    ...
    public function listData($where = [],$field = '*',$page = 0,$order = 'id asc',$group = ''){
        $cache_list = Cache::get(self::$cache_area);
        if(!$cache_list){
            $list =Db::table($this->table)->field($field)->where($where)->page($page)->group($group)->order($order)->cache(self::$cache_area)->select();
            $cache_list = array();
            foreach($list as $v){
                $cache_list[$v['id']] = $v;
            }
            Cache::set(self::$cache_area,$cache_list);
        }
        return $cache_list;
    }
